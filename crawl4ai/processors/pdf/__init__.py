...
class PDFContentScrapingStrategy(ContentScrapingStrategy):
    ...
    def scrap(self, url: str, html: str, **params) -> ScrapingResult:
        ...
        try:
            # Checkk page count before processing
            page_count = self._get_page_count(Path(pdf_path))
            if self.max_pages and page_count > self.max_pages:
                if self.logger:
                    self.logger.warning(f"Skipping PDF with {page_count} pages (max allowed: {self.max_pages})")

                # Return minimal result for large PDFs
                return ScrapingResult(
                    cleaned_html=f"""
                    <html>
                        <head><meta name="pdf-pages" content="{page_count}"></head>
                        <body>
                            <div class="pdf-skipped">
                            <h2>PDF Skipped - Too Many Pages</h2>
                                <p>PDF has {page_count} pages, but max allowed is {self.max_pages}.</p>
                                <p>Skipped to prevent processing timeout.</p>
                            </div>
                        </body>
                    </html>
                    """,
                    ...
                    metadata={
                        "pages": page_count,
                        "skipped": True,
                        "reason": f"Too many pages ({page_count} > {self.max_pages})",
                    },
                )
            if self.logger:
        ...
        """
            ...
            return ScrapingResult(
                cleaned_html=cleaned_html, success=True, media=media, links=links, metadata=asdict(result.metadata)
            )
        finally:
    ...
    def _get_page_count(self, pdf_path: Path) -> int:
        """
        Gets page count without processing the PDF
        """
        try:
            ...
            # Assume too big if cannot get page count
